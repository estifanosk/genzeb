...
function writeLedger(rows: Array<{ id: string; date: string; merchant: string; amount: number; account?: string }>) {
  ...
  const lines = rows.map(r =>
    `${r.id},${r.account ?? 'Checking'},${r.date},,desc,${r.merchant},${r.amount},USD,file.csv,hash,2026-01-01T00:00:00Z`
  )
...
describe('materializeTransactions + queryTransactions', () => {
  it('materializes ledger rows into queryable transactions', () => {
    ...
    const { transactions, total } = queryTransactions(dir, { filters: {}, limit: 10, offset: 0, sortBy: 'date', sortOrder: 'desc' })
    expect(total).toBe(2)
  ...
  it('search filter matches merchant', () => {
    ...
    materializeTransactions(dir)
    const { transactions } = queryTransactions(dir, { filters: { search: 'starbucks' }, limit: 10, offset: 0, sortBy: 'date', sortOrder: 'asc' })
  ...
  it('uncategorized filter returns only transactions without a category', () => {
    ...
    materializeTransactions(dir)
    const { transactions } = queryTransactions(dir, { filters: { uncategorized: true }, limit: 10, offset: 0, sortBy: 'date', sortOrder: 'asc' })
    expect(transactions).toHaveLength(1)
    expect(transactions[0].id).toBe('tx2')
  })
  ...
  it('pagination returns the correct slice', () => {
    ...
    materializeTransactions(dir)
    const page1 = queryTransactions(dir, { filters: {}, limit: 2, offset: 0, sortBy: 'date', sortOrder: 'asc' })
    const page2 = queryTransactions(dir, { filters: {}, limit: 2, offset: 2, sortBy: 'date', sortOrder: 'asc' })
...
describe('agent changes → ai_edited flag', () => {
  it('marks a transaction ai_edited when an agent change is applied', () => {
    ...
    appendChangeRow(dir, { transaction_id: 'tx1', change_type: 'set_category', value: 'Food', agent: 'claude' })
    materializeTransactions(dir)
    const { transactions } = queryTransactions(dir, { filters: {}, limit: 10, offset: 0, sortBy: 'date', sortOrder: 'asc' })
  ...
  it('applies set_category, set_merchant, set_notes changes', () => {
    ...
    appendChangeRow(dir, { transaction_id: 'tx1', change_type: 'set_category', value: 'Dining' })
    appendChangeRow(dir, { transaction_id: 'tx1', change_type: 'set_merchant', value: 'New Name' })
    appendChangeRow(dir, { transaction_id: 'tx1', change_type: 'set_notes', value: 'check this' })
    materializeTransactions(dir)
    const { transactions } = queryTransactions(dir, { filters: {}, limit: 10, offset: 0, sortBy: 'date', sortOrder: 'asc' })
